(() => {
  ...
  const ROUTE_ORDER = { "Soča-Tal": 0, "Istrien": 1 };
  ...
  function routeRank(camp) {
    return ROUTE_ORDER[camp.region] ?? 99;
  }
  ...
  function renderCockpit() {
    ...
    cockpit.innerHTML = `
      ...
      <div class="cockpit-links">
        ${phoneAction}
        <a href="${escapeHtml(camp.website)}" target="_blank" rel="noopener noreferrer" data-external>🌐 Website</a>
        <a href="${escapeHtml(camp.maps)}" target="_blank" rel="noopener noreferrer" data-external>📍 Karte</a>
      </div>
      ...
      return `
        <article class="camp-card compact" data-card="${escapeHtml(c.id)}">
          ...
          <div class="contact-grid compact-actions">
            ${phoneButton}
            <a href="${escapeHtml(c.website)}" target="_blank" rel="noopener noreferrer" data-external>🌐 Website</a>
            <a href="${escapeHtml(c.maps)}" target="_blank" rel="noopener noreferrer" data-external>📍 Karte</a>
          </div>
  ...
  function showPage(id, targetId = null) {
  ...
  function initEvents() {
    ...
    document.querySelectorAll("[data-go]").forEach(btn => {
      btn.addEventListener("click", () => showPage(btn.dataset.go, btn.dataset.go === "camping" ? "callCockpit" : null));
    });
    ...
    document.addEventListener("click", event => {
      ...
      if (opened) {
        ...
        toast("Im Browser geöffnet – über den App-Umschalter zurück");
      } else {
        toast("Browser blockiert – Website wird direkt geöffnet");
    ...
    document.getElementById("campForm").addEventListener("submit", saveCampFromDialog);
    document.getElementById("closeCampDialog").addEventListener("click", () => document.getElementById("campDialog").close());
    document.getElementById("campDialog").addEventListener("click", e => {
